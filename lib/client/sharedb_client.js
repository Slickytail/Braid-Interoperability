...
class SocketRewriter extends WebSocket {
    ...
    send(data) {
        ...
        this.log_send(data, JSON.stringify(message));
        super.send(JSON.stringify(message));
    }

    log_send(orig, trans) {
        if (this.console)
            this.console(orig, trans, true)
        else {
            console.log(`Sending:    ${orig}`)
            console.log(`Rewrote to: ${trans}`, " \n ")
        }
    }
    ...
    set onmessage(handler) {
        super.onmessage = function(e) {
            if (e.data) {
                ...
                if (m.data != "{}") {
                    var orig = JSON.parse(e.data).a ? "(No Original)" : e.data;
                    // If we spoofed this message, then the original is "nothing"
                    if (this.console)
                        this.console(orig, m.data, false);
                    else {
                        console.log(`Receieved:  ${orig}`)
    ...
    _rewrite_incoming_op(data, d) {
        ...
        if (s != -1) {
            ...
            if (v)
                ...
                var ack = `{"ack": "val", "version": "${data.version}"}`
                setTimeout(() => {
                    super.send(ack);
                    this.log_send("(No Original)", ack);
                }, 10);
            }
        }
    }
...
class OPTimeDag {
    ...
    add_op(vid, parents, op) {
        if (this.has(vid)) {
            global.sync(false);
            console.error(`Can't add pre-existing op ${vid}`)
            return;
        }
