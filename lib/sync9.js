    ...
    patches.forEach(change => {
        ...
        each(parse.keys, (key, i) => {
            ...
            if (typeof(key) == 'string' && cur.t == 'obj') {
                ...
                } else throw 'Bad patch JSON'
            } else {
                throw 'Unknown key'
            }
        })
        if (!parse.range) {
            if (cur.t != 'val') throw 'No range'
            sync9_space_dag_add_version(cur.S, vid, [[0, 0, [sync9_wrap(parse.val, vid)]]], is_anc)
        } else {
            if (cur.t == 'val') cur = sync9_space_dag_get(cur.S, 0, is_anc)
            if (parse.val instanceof Array && cur.t != 'arr') throw "Can't patch array into non-array"
            if (parse.val instanceof String && cur.t != 'str') throw "Can't patch string into non-string"
...
function sync9_wrap(x, vid) {
    ...
    } else throw `Can't wrap object of type ${typeof(x)}`
}
