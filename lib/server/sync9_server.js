...
var TransparentJSONSocket = require('./socket');
...
class Server {
    ...
    constructor(ready, config) {
        ...
        this.config = {prune: true, prune_freq: 10}
    ...
    _leave(uid) {
        ...
        Object.keys(this.prune_info)
        .forEach(x => {delete this.prune_info[x].sent[uid];
                       delete this.prune_info[x].acked[uid];});
        this.prune();
    }
    ...
    _route_outgoing_message(message, destination) {
        ...
        if (p.stream.readyState != 1)
            p.stream.write(message)
        else
            this._leave(destination)
    }
}
